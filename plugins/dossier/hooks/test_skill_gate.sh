...
cleanup() { rm -rf "$TMP" /tmp/ds-skill-gate-tg*; }
trap cleanup EXIT
...
out="$(run_gate "$(payload review tg-a)")" || fail "gate must exit 0 on builtin"
...
out="$(run_gate "$(payload review tg-a)")" || fail "gate must exit 0 on dedup"
[[ -z "$out" ]] || fail "second fire same session must dedup to silence"

out="$(run_gate "$(payload security-review tg-a)")" || fail "gate must exit 0 on second builtin"
printf '%s' "$out" | grep -q 'additionalContext' || fail "different builtin same session must still fire"

out="$(run_gate "$(payload caveman:caveman-review tg-b)")" || fail "gate must exit 0 on namespaced"
[[ -z "$out" ]] || fail "namespaced plugin skill must stay silent"

out="$(run_gate "$(payload foo tg-c)")" || fail "gate must exit 0 on non-builtin"
[[ -z "$out" ]] || fail "non-builtin skill must stay silent"

out="$(run_gate "$(printf '{"hook_event_name":"UserPromptExpansion","command":"simplify","cwd":"%s","session_id":"tg-d"}' "$TMP")")" || fail "gate must exit 0 on typed path"
...
out="$(run_gate "$(payload review tg-e)")" || fail "gate must exit 0 without lock"
[[ -z "$out" ]] || fail "no in-flight build must stay silent"

rm "$TMP/.scratchpad/INDEX.md"
out="$(run_gate "$(payload review tg-f)")" || fail "gate must exit 0 without INDEX"
