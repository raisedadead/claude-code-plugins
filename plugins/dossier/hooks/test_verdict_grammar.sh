...
grep -q 'DOUBT: FAILURES' "$TMP/d.tbl" || fail "table must carry the doubter verdict line"
grep -q 'NO FAILURE FOUND' "$TMP/d.tbl" || fail "table must carry the doubter alternative"
grep -q 'REVIEW: PASS' "$TMP/d.tbl" || fail "table must carry the reviewer verdict line"
grep -q 'CHANGES' "$TMP/d.tbl" || fail "table must carry the reviewer alternative"
