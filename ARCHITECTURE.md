...
**Subagents run fresh and read-only.** `dossier-scout` investigates; `dossier-reviewer` reviews a staged diff before commit; `whetstone-doubter` attacks a plan before code exists. Each receives an artifact-only mission — the diff, the plan, the contract — and never the parent's reasoning. That is the whole point: an agent that never saw the justification falsifies the work instead of rationalising it. Read-only is enforced by tool grant for `whetstone-doubter` alone (`Read, Grep, Glob`); `dossier-scout` and `dossier-reviewer` are granted `Bash` and are held read-only by their prompts — model-judgment, not code. Narrowing those two grants is unclaimed work, not a property to rely on today.
...
| Karpathy, [nanochat](https://github.com/karpathy/nanochat) and [nanoGPT](https://github.com/karpathy/nanoGPT) | "One single dial of complexity… no giant configuration objects, model factories, or if-then-else monsters." Two repos, one standing convention. Direct lineage for ceremony net-negative.                                                                                                                      |
