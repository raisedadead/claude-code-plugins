...
**Subagents run fresh and read-only.** `dossier-scout` investigates; `dossier-reviewer` reviews a staged diff before commit; `whetstone-doubter` attacks a plan before code exists. Each receives an artifact-only mission — the diff, the plan, the contract — and never the parent's reasoning. That is the whole point: an agent that never saw the justification falsifies the work instead of rationalising it. All three refuse writes.
